        ...
        response = await model.ainvoke([
            SystemMessage(
                ...
                1. **conversation** - 普通对话
                   ...
                   - 功能咨询："你能做什么"、"怎么使用"
                   - 闲聊内容
                   - 关于RAG检索的说明性消息："Brief数据已上传，请开始分析并进行文案库检索"
                ...
                4. **search** - 搜索需求
                   - 搜索请求："搜索一下..."、"查找..."
                   - 参考需求："找些参考资料"、"看看竞品"
                   - 注意：不包括"RAG检索"、"文案库检索"等内部系统功能

                **重要规则：**
                - 如果用户输入是问候、自我介绍询问或功能咨询，返回 "conversation"
                - 如果用户明确提供产品信息、要求创作内容或生成博主人设，返回 "note_creation"
                - 如果用户要求搜索外部资料，返回 "search"
                - 如果是关于"Brief数据已上传"、"RAG检索"、"文案库检索"等系统内部消息，返回 "conversation"
